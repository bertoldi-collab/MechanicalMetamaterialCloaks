...
class ForwardProblem:
    """
    Forward problem for the static cloaking of a void in a quad metamaterial.
    BCs:
        - Left column clamped.
    ...
    # Mother Geometry
    horizontal_vertical_shifts_mg: Any = (
        None  # one of the two argument needs to be fill
    )
    initial_angle: Optional[float] = None

    # Problem name
    name: str = "quads_static_cloaking"
    ...
    def setup(self) -> None:
        ...
        self.mother_geometry = QuadGeometry(
        ...
        if self.initial_angle == None:
            ...
            _init_block_centroids_mg = self.block_centroids_mg(
                *self.horizontal_vertical_shifts_mg
            )
        else:
            self.horizontal_vertical_shifts_mg = (
                self.mother_geometry.get_design_from_rotated_square(
        ...
        # cloaked geometry
        self.cloaked_geometry = CloakQuadGeometry(
        ...
        mgIDs_blocks_last_column = (
            jnp.arange(0, self.mother_geometry.n2_blocks)
            * self.mother_geometry.n1_blocks
            + self.mother_geometry.n1_blocks
            - 1
        )
        ...
        mgIDs_blocks_first_column = (
            jnp.arange(0, self.n2_blocks) * self.mother_geometry.n1_blocks
        )
        ...
        # TODO: determine if moving_blocks_ids_mg is useful
        moving_blocks_ids_mg = jnp.array(
            [
                j + i * self.n1_blocks
        ...
        ids_driven_block_DOF_pairs_cg_first_column = jnp.array(
            [
                ...
                for k in range(0, self.n2_blocks)
            ]
        )
        ids_clamped_block_DOF_pairs_cg_last_column = jnp.array(
            [
                self.cloaked_geometry.grid_convert_mgIDs_to_cgIDs[k, self.n1_blocks - 1]
                for k in range(self.n2_blocks)
            ]
        )
        ...
        # TODO: determine if moving_blocks_ids_cg is usefull
        moving_blocks_ids_cg = []
        for i in range(self.n2_blocks):
            for j in range(1, self.n1_blocks - 1):
                if self.cloaked_geometry.grid_convert_mgIDs_to_cgIDs[i, j] != -1:
                    moving_blocks_ids_cg.append(
                        self.cloaked_geometry.grid_convert_mgIDs_to_cgIDs[i, j]
                    )
        self.moving_blocks_ids_cg = jnp.array(moving_blocks_ids_cg)
        ...
        def plot_sketch():
            ...
            blocks_for_plot = self.block_centroids_cg(
                *self.horizontal_vertical_shifts_mg
            )
        ...
        def get_control_params_cg_all_shifts(horizontal_vertical_shifts):
        ...
        # Control params fn
        def get_control_params_cg(horizontal_vertical_shifts_cloak_area):
            horizontal_shifts, vertical_shifts = cloak_to_all_shifts(
                horizontal_vertical_shifts_cloak_area
            )
            return get_control_params_cg_all_shifts(
                (horizontal_shifts, vertical_shifts)
            )

        def forward_all_shifts(horizontal_vertical_shifts):
            control_params_cg = get_control_params_cg_all_shifts(
        ...
        def forward(horizontal_vertical_shifts_cloak_area):
            ...
            return forward_all_shifts((horizontal_shifts, vertical_shifts))
    ...
    def _force_displacement(
        ...
        if self.is_setup:
            ...
            block_DOF_pairs = self.driven_block_DOF_pairs_cg[: self.n2_blocks]
            force_history = vmap(
                lambda u: jnp.sum(
    ...
    def force_displacement_cloak(
        ...
        return self._force_displacement(
            solution_data, self.get_control_params_cg(horizontal_vertical_shifts_cloak)
        )
    ...
    def reaction_force_history(
        ...
        if geometry_type == "mg":
            ...
            if column_type == "first":
                ids_DOF_pairs_mg = (
                    jnp.arange(0, self.forward_problem.n2_blocks)
                    * self.forward_problem.n1_blocks
                )
            ...
            block_DOF_pairs_mg_targeted_column = jnp.array(
                [
                    ...
                    jnp.array([0] * self.forward_problem.n2_blocks),
                ]
            ).T
        ...
        # reqction force for the cloacked geometry
            ...
            n1_blocks = self.forward_problem.mother_geometry.n1_blocks
            n2_blocks = self.forward_problem.mother_geometry.n2_blocks
