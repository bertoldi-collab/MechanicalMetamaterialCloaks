...
from mechanicalmetamaterialcloaks.geometry import QuadGeometry, RotatedSquareGeometry, compute_xy_limits
from mechanicalmetamaterialcloaks.plotting import generate_polygons
...
def generate_chamfer_lines(
    ...
    for block_id, block in enumerate(block_nodes):
        for node_local_id, node in enumerate(block):
            ...
            if director != None:
                prev_node, next_node = block[node_local_id-1], block[jnp.mod(node_local_id+1, len(block))]
                ...
                chamfer_point_1 = node + (prev_node-node) / jnp.linalg.norm(prev_node-node) * chamfer_depth / min_cos_1
                chamfer_point_2 = node + (next_node-node) / jnp.linalg.norm(next_node-node) * chamfer_depth / min_cos_2
                chamfer_lines_data.append(jnp.array([chamfer_point_1, chamfer_point_2]))
...
def generate_slot_lines_straight(
    ...
    slot_lines_data = vmap(slot_line, in_axes=(0, 0))(nodes[bond_connectivity], reference_bond_vectors)
...
def generate_slot_lines_centroid(
    ...
    centroid_node_vectors_flattened = centroid_node_vectors.reshape((n_blocks*n_block_nodes, 2))
...
def generate_slot_lines_bisectrix(
    ...
    edge_unit_vectors = (edge_vectors.T / jnp.linalg.norm(edge_vectors, axis=-1)).T
...
def generate_continuous_bond_lines_centroid(
    ...
    if offset_type == "ratio":
        centroid_node_vectors_shifted = (1-offset_size) * centroid_node_vectors_flattened
    else:
    ...
    bond_lines_data = vmap(bond_line, in_axes=(0, 0))(
        nodes[bond_connectivity],
        (centroid_node_vectors_flattened-centroid_node_vectors_shifted)[bond_connectivity]
    )
    ...
    if path_orientation == "column":
        ...
        internal_segments_connectivity = [[[0, 3], [1, 2]] if n2 % 2 == 0 else [
            [0, 1], [2, 3]] for n1 in range(n1_blocks) for n2 in range(n2_blocks)]
    else:
        raise ValueError("Wrong path orientation!")

    internal_segments = []
    for block, ic in zip(block_nodes_shifted, internal_segments_connectivity):
        internal_segments.extend(block[jnp.array(ic)])

    internal_segments = jnp.array(internal_segments)
    ...
    if n1_blocks % 2 == 1:
        bottom_lines = jnp.concatenate((bottom_lines, jnp.array([bottom_lines[-1]])))
        right_lines = jnp.concatenate((right_lines, jnp.array([right_lines[-1]])))

    if n2_blocks % 2 == 1:
        left_lines = jnp.concatenate((jnp.array([left_lines[0]]), left_lines))
        top_lines = jnp.concatenate((jnp.array([top_lines[0]]), top_lines))
    all_bond_lines = all_bond_lines + list(bottom_lines)+list(top_lines)+list(left_lines)+list(right_lines)
    ...
    all_bond_lines = all_bond_lines + list(bottom_closing_lines) + list(top_closing_lines) + \
        list(left_closing_lines) + list(right_closing_lines) + [corner_closing_lines]
...
def generate_blocks_continuous_bond_lines_drawing(
    ...
    xlim, ylim = compute_xy_limits((block_centroids[:, None] + centroid_node_vectors).reshape(-1, 2))
    ...
    out_path = Path(out_file)
    out_path.parent.mkdir(parents=True, exist_ok=True)  # Make sure parents exist
    fig.savefig(str(out_path), dpi=600, transparent=True)
    plt.close(fig)
    print("Saved at " + str(out_path))


def generate_blocks_continuous_bond_lines_drawing_seperate(
        block_centroids: jnp.ndarray,
        centroid_node_vectors: jnp.ndarray,
        bond_connectivity: jnp.ndarray,
        reference_bond_vectors: jnp.ndarray,
    ...
    fig, axes = plt.subplots()
    fig_blocks, axes_blocks = plt.subplots()
    fig_bonds, axes_bonds = plt.subplots()
    ...
    block_lines_blocks = generate_block_lines(block_centroids, centroid_node_vectors)
    bond_lines_bonds = generate_continuous_bond_lines_centroid(
        block_centroids,
        centroid_node_vectors,
        bond_connectivity,
        reference_bond_vectors,
    ...
    axes_blocks.add_collection(block_lines_blocks)
    if block_hole_size is not None:
        central_holes = generate_central_holes(
            block_centroids,
            centroid_node_vectors,
        ...
        axes_blocks.add_collection(central_holes)
    axes_blocks.autoscale()
    axes_blocks.axis("equal")
    axes_blocks.axis("off")

    axes_bonds.add_collection(bond_lines_bonds)
    axes_bonds.autoscale()
    axes_bonds.axis("equal")
    axes_bonds.axis("off")
    ...
    out_path_blocks = Path(out_file.replace('.svg', '_blocks.svg'))
    out_path_blocks.parent.mkdir(parents=True, exist_ok=True)  # Make sure parents exist
    fig_blocks.savefig(str(out_path_blocks), format='svg', transparent=True)
    plt.close(fig_blocks)
    print("Saved blocks at " + str(out_path_blocks))

    out_path_bonds = Path(out_file.replace('.svg', '_bonds.svg'))
    out_path_bonds.parent.mkdir(parents=True, exist_ok=True)  # Make sure parents exist
    fig_bonds.savefig(str(out_path_bonds), format='svg', transparent=True)
    plt.close(fig_bonds)
    print("Saved bonds at " + str(out_path_bonds))
...
def generate_shim_lines(n_shims: int, length: float, width: float, hole_size: float, n_rows: int = 1):
    ...
    for i in range(n_rows):
        for j in range(cols[i]):
            rectangles.append(Rectangle((0. + j*1.1*width, 0. + i*1.1*length), width, length))
            circles.append(
                Circle((0.25*width + j*1.1*width, 0.1*length + i*1.1*length), radius=hole_size/2)
            )  # bottom hole
            circles.append(
                Circle((0.75*width + j*1.1*width, 0.1*length + i*1.1*length), radius=hole_size/2)
            )  # bottom hole
            circles.append(
                Circle((0.25*width + j*1.1*width, (1-0.1)*length + i*1.1*length), radius=hole_size/2)
            )  # top hole
            circles.append(
                Circle((0.75*width + j*1.1*width, (1-0.1)*length + i*1.1*length), radius=hole_size/2)
            )  # top hole
...
def generate_shims_cut_drawing(n_shims: int, length: float, width: float, hole_size: float, out_file: str, n_rows: int = 1):
    ...
    patches_boundary, patches_holes = generate_shim_lines(n_shims, length, width, hole_size, n_rows)
    fig, axes = plt.subplots()
...
def generate_grip_cuts(geometry: RotatedSquareGeometry, angle, hinge_length, grip_width, grip_lateral_spacing, hole_size: Optional[float] = None):
    """
    Generates cuts for grips
    """
    ...
    grip_top_top_left_corner = jnp.array([xlim[0], ylim[1]]) + hinge_length*e2 - grip_lateral_spacing*e1
    grip_top_bottom_left_corner = jnp.array([xlim[0], ylim[0]]) - grip_lateral_spacing*e1
    grip_top = Polygon(
        jnp.array([
    ...
    grip_bottom_top_right_corner = jnp.array([xlim[1], ylim[1]]) + grip_lateral_spacing*e1
    grip_bottom_bottom_right_corner = jnp.array([xlim[1], ylim[0]]) - hinge_length*e2 + grip_lateral_spacing*e1
    grip_bottom_bottom_left_corner = jnp.array([xlim[0], ylim[0]]) - hinge_length*e2
    grip_bottom = Polygon(
        jnp.array([
    ...
    if hole_size is not None:
        holes = []
        holes.append(
            Circle((xlim.mean() + 20., ylim[1] + hinge_length + 0.6*grip_width), radius=hole_size/2)
        )  # Top
        holes.append(Circle((xlim.mean() - 20., ylim[1] + hinge_length + 0.6*grip_width), radius=hole_size/2))
        holes.append(
            Circle((xlim.mean() + 20., ylim[0] - hinge_length - 0.6*grip_width), radius=hole_size/2)
        )  # Bottom
        holes.append(
            Circle((xlim.mean() - 20., ylim[0] - hinge_length - 0.6*grip_width), radius=hole_size/2)
        )
        holes.append(
            Circle((xlim[0] - grip_lateral_spacing - 0.5*grip_width, ylim.mean() + 20.), radius=hole_size/2)
        )  # Left
        holes.append(
            Circle((xlim[0] - grip_lateral_spacing - 0.5*grip_width, ylim.mean() - 20.), radius=hole_size/2)
        )
        holes.append(
            Circle((xlim[1] + grip_lateral_spacing + 0.5*grip_width, ylim.mean() + 20.), radius=hole_size/2)
        )  # Right
        holes.append(
            Circle((xlim[1] + grip_lateral_spacing + 0.5*grip_width, ylim.mean() - 20.), radius=hole_size/2)
        )
...
def generate_grippable_sample_cuts_drawing(
        ...
        slot_type: Literal["straight", "centroid", "bisectrix"] = "straight"):
    """
    Generates sample cuts with top/bottom grip pieces.
    """
    # NOTE: This only works for RotatedSquareGeometry!
    ...
    block_lines = generate_block_lines(block_centroids(angle), centroid_node_vectors(angle))
    slot_lines = generate_slot_lines(
...
def generate_clamped_sample_cuts_drawing(
    ...
    clamped_block_ids_bl = jnp.concatenate([
        ...
        jnp.array([0+i*geometry.n1_blocks for i in range(1, n_blocks_clamped_corners)])
    ])
    clamped_block_ids_br = jnp.concatenate([
        jnp.arange(geometry.n1_blocks-n_blocks_clamped_corners, geometry.n1_blocks),
        jnp.array([(i+1)*geometry.n1_blocks-1 for i in range(1, n_blocks_clamped_corners)])
    ])
    clamped_block_ids_tr = jnp.concatenate([
        jnp.arange(geometry.n_blocks-n_blocks_clamped_corners, geometry.n_blocks),
        jnp.array([geometry.n_blocks-i*geometry.n1_blocks-1 for i in range(1, n_blocks_clamped_corners)])
    ])
    ...
    block_ids_inner = jnp.setdiff1d(jnp.arange(geometry.n_blocks), block_ids_clamped_corners)
    bond_ids_corners = jnp.concatenate([
        jnp.arange(0, n_blocks_clamped_corners-1),
        jnp.arange(geometry.n1_blocks-n_blocks_clamped_corners, geometry.n1_blocks-1),
        ...
        jnp.array(
            [geometry.n1_blocks-1+i*geometry.n1_blocks for i in jnp.arange(n_blocks_clamped_corners-1)]
        ) + (geometry.n1_blocks-1)*geometry.n2_blocks,
        jnp.array(
    ...
    bond_ids_inner = jnp.setdiff1d(jnp.arange(len(bond_connectivity)), bond_ids_corners)

    block_lines_inner = generate_block_lines(block_centroids[block_ids_inner], centroid_node_vectors[block_ids_inner])
    slot_lines = generate_slot_lines(
        block_centroids,
        centroid_node_vectors,
    ...
    # Attachment holes
    hole_shift_x = (25.4 - jnp.mod((geometry.n1_blocks-1)*geometry.spacing, 25.4))/2  # Based on 1" breadboard
    hole_shift_y = (25.4 - jnp.mod((geometry.n2_blocks-1)*geometry.spacing, 25.4))/2  # Based on 1" breadboard
    hole_position_bl = (-hole_shift_x, -hole_shift_y)
    hole_position_br = ((geometry.n1_blocks-1)*geometry.spacing + hole_shift_x, -hole_shift_y)
    hole_position_tl = (-hole_shift_x, (geometry.n2_blocks-1) * geometry.spacing+hole_shift_y)
    ...
    xlim_bl, ylim_bl = compute_xy_limits(blocks_bl.reshape((2*n_blocks_clamped_corners-1)*geometry.n_npb, 2))
    attachment_bl_points = jnp.array(
        shape_ops.unary_union([
            *(shape_geom.Polygon(block) for block in blocks_bl),
            shape_geom.box(xlim_bl[0], ylim_bl[0], xlim_bl[1]-0.25*geometry.spacing, ylim_bl[0]+leg_width),
            shape_geom.box(xlim_bl[0], ylim_bl[0], xlim_bl[0]+leg_width, ylim_bl[1]-0.25*geometry.spacing),
            shape_geom.box(
    ...
    xlim_br, ylim_br = compute_xy_limits(blocks_br.reshape((2*n_blocks_clamped_corners-1)*geometry.n_npb, 2))
    attachment_br_points = jnp.array(
        shape_ops.unary_union([
            *(shape_geom.Polygon(block) for block in blocks_br),
            shape_geom.box(xlim_br[0]+0.25*geometry.spacing, ylim_br[0], xlim_br[1], ylim_br[0] + leg_width),
            shape_geom.box(xlim_br[1], ylim_br[0], xlim_br[1] - leg_width, ylim_br[1]-0.25*geometry.spacing),
            shape_geom.box(
    ...
    xlim_tl, ylim_tl = compute_xy_limits(blocks_tl.reshape((2*n_blocks_clamped_corners-1)*geometry.n_npb, 2))
    attachment_tl_points = jnp.array(
        shape_ops.unary_union([
            *(shape_geom.Polygon(block) for block in blocks_tl),
            shape_geom.box(xlim_tl[0], ylim_tl[1], xlim_tl[1]-0.25*geometry.spacing, ylim_tl[1] - leg_width),
            shape_geom.box(xlim_tl[0], ylim_tl[1], xlim_tl[0] + leg_width, ylim_tl[0]+0.25*geometry.spacing),
            shape_geom.box(
    ...
    xlim_tr, ylim_tr = compute_xy_limits(blocks_tr.reshape((2*n_blocks_clamped_corners-1)*geometry.n_npb, 2))
    attachment_tr_points = jnp.array(
        shape_ops.unary_union([
            *(shape_geom.Polygon(block) for block in blocks_tr),
            shape_geom.box(xlim_tr[1], ylim_tr[1], xlim_tr[0]+0.25*geometry.spacing, ylim_tr[1] - leg_width),
            shape_geom.box(xlim_tr[1], ylim_tr[1], xlim_tr[1] - leg_width, ylim_tr[0]+0.25*geometry.spacing),
            shape_geom.box(
    ...
    xlim, ylim = compute_xy_limits(
        jnp.concatenate([attachment_bl_points, attachment_br_points, attachment_tl_points, attachment_tr_points])
    )
...
def generate_clamped_sample_cuts_continuous_bond_drawing(
        geometry: Union[RotatedSquareGeometry, QuadGeometry],
        block_centroids: jnp.ndarray,
        centroid_node_vectors: jnp.ndarray,
        bond_connectivity: jnp.ndarray,
