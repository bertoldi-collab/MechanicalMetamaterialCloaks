...
import cv2
import numpy as np
from mechanicalmetamaterialcloaks.geometry import compute_xy_limits
from mechanicalmetamaterialcloaks.plotting import generate_animation
from mechanicalmetamaterialcloaks.utils import SolutionData, load_data, save_data
from scripts.tracking.tracking_points import track_points
from scripts.tracking.utils import (collect_as, compute_block_displacement_from_markers, compute_blocks_displacement_history_from_markers, compute_blocks_displacement_history_from_markers_incremental,
...
def mark_reference_frame(
    ...
    for masked_area in masked_areas:
        image[ROI_XY[1][0]: ROI_XY[1][1], ROI_XY[0][0]: ROI_XY[0][1]][masked_area[1]
                                                                      [0]: masked_area[1][1], masked_area[0][0]: masked_area[0][1]] = 255
    # Get contours
...
def tracking_better(
    ...
    Args:
        ...
        template_update_rate (int): Update rate for the reference frame (in number of frames). Default is 0 (i.e. no update).
        search_window_update_rate (int): Update rate for the search window (in number of frames). Default is 1 (i.e. update every frame).
        print_frame_number (bool): Whether to print the frame number. Default is False.
        show_tracked_frame (bool): Whether to show the tracked frame. Default is False.

    Returns:
        SolutionData: NamedTuple containing information of the tracked blocks.
    """
    # NOTE: max_angle_change and max_displacement_change are currently ignored.
    ...
    video_capture.set(cv2.CAP_PROP_POS_FRAMES, startVideo)
    if endVideo == -1:
        endVideo = int(video_capture.get(cv2.CAP_PROP_FRAME_COUNT))-1
    ...
    n_blocks = len(block_centroids)
    block_centroids = block_centroids * calib_xy
    markers = np.array(marked_blocks.reshape(-1, 2)).astype(np.float64)

    # Perform marker tracking over the entire video
    markers_history = track_points(
        video_path,
        markers=markers,
        ROI_X=ROI_X,
        ROI_Y=ROI_Y,
        frame_range=(startVideo, endVideo),
        step_size=step_size,
        search_window_size=search_window_size,
        marker_template_size=marker_template_size,
        upscaling_factor=upscaling_factor,
        template_update_rate=template_update_rate,
        search_window_update_rate=search_window_update_rate,
        print_frame_number=print_frame_number,
        show_tracked_frame=show_tracked_frame,
    )
    # Reshape markers to blocks
    marked_blocks_history = markers_history.reshape(markers_history.shape[0], n_blocks, -1, 2)
    # Compute block displacements
    block_displacement_history = compute_blocks_displacement_history_from_markers(
        marked_blocks_history[0],  # Initial frame wrt which displacements are computed
        marked_blocks_history,  # Marker history
        calib_xy,  # Calibration constants
    )
    # Rotations can only be computed by accumulating incremental frame-by-frame rotations
    # Compute incremental frame-by-frame block displacements
    block_displacement_history_incremental = compute_blocks_displacement_history_from_markers_incremental(
        marked_blocks_history[:-1],  # Previous frames
        marked_blocks_history[1:],  # Current frames
        calib_xy,  # Calibration constants
    )
    # Accumulate incremental rotations
    block_rotation_history = np.cumsum(block_displacement_history_incremental[:, :, 2], axis=0)
    # Add initial rotation with zeros
    block_rotation_history = np.concatenate((np.zeros((1, n_blocks)), block_rotation_history), axis=0)
    # Update block displacement history with rotations
    block_displacement_history = block_displacement_history.at[:, :, 2].set(block_rotation_history)
    # Compute block velocities
    block_velocity_history = np.diff(block_displacement_history, axis=0) * framerate / step_size
    solution = np.zeros((len(block_velocity_history), 2, n_blocks, 3))
    solution[:, 0, :, :] = block_displacement_history[:-1]
    solution[:, 1, :, :] = block_velocity_history
    # Replace NaNs with interpolated values.
    # solution = interpolate_nans(solution)
    ...
    timepoints = np.arange(startVideo, endVideo + 1, step_size) / framerate
    timepoints = timepoints[:-1]    # Remove last timepoint to match solution shape
