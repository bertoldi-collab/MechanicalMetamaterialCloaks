...
from mechanicalmetamaterialcloaks.geometry import polygon_centroid as polygon_centroid_jax
from mechanicalmetamaterialcloaks.geometry import angle_between_unit_vectors as angle_between_unit_vectors_jax
...
def compute_block_displacement_from_markers_jax(previous_markers, current_markers, calib_xy=(1, 1)):
    """Compute the displacement of the block from the marker positions.
    ...
    # Compute xy displacement of the centroid
    previous_centroid = polygon_centroid_jax(previous_markers)
    current_centroid = polygon_centroid_jax(current_markers)
    xy_displacement = (current_centroid - previous_centroid) * jnp.array(calib_xy)

    # Compute angle change using the longest edge
    previous_edges = jnp.roll(previous_markers, 1, axis=0) - previous_markers
    current_edges = jnp.roll(current_markers, 1, axis=0) - current_markers
    previous_edge_lengths = jnp.linalg.norm(previous_edges, axis=-1)
    current_edge_lengths = jnp.linalg.norm(current_edges, axis=-1)
    # NOTE: The angle change is weighted by the length of the edge. This makes rotation tracking more robust to noise in the marker positions.
    theta_displacement = angle_between_unit_vectors_jax(
        previous_edges.T/previous_edge_lengths,
        current_edges.T/current_edge_lengths
    ) @ (current_edge_lengths/current_edge_lengths.sum())

    block_displacement = jnp.array([xy_displacement[0], xy_displacement[1], theta_displacement])

    return block_displacement


compute_blocks_displacement_from_markers = vmap(lambda previous_markers_block, current_markers_block, calib_xy: compute_block_displacement_from_markers_jax(
    previous_markers_block, current_markers_block, calib_xy), in_axes=(0, 0, None))

compute_blocks_displacement_history_from_markers = vmap(lambda previous_markers_blocks, current_markers_blocks, calib_xy: compute_blocks_displacement_from_markers(
    previous_markers_blocks, current_markers_blocks, calib_xy), in_axes=(None, 0, None))

compute_blocks_displacement_history_from_markers_incremental = vmap(lambda previous_markers_blocks, current_markers_blocks, calib_xy: compute_blocks_displacement_from_markers(
    previous_markers_blocks, current_markers_blocks, calib_xy), in_axes=(0, 0, None))
