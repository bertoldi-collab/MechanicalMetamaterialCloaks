...
from mechanicalmetamaterialcloaks.geometry import DOFsInfo, Geometry, rotation_matrix


def _block_to_node_displacement(
    block_displacement: jnp.ndarray, centroid_node_vectors: jnp.ndarray
):
    ...
    node_displacement = block_centroid_displacement + jnp.dot(
        rotation_matrix(block_rotation) - jnp.eye(2), centroid_node_vectors
    )
...
def build_constrained_kinematics(
    ...
    def constrained_kinematics(
        free_DOFs: jnp.ndarray, t, constraint_params: Dict = dict()
    ):
