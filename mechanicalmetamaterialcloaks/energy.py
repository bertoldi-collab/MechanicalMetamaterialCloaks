...
def simple_spring_energy(
    nodal_DOFs: Tuple[jnp.ndarray, jnp.ndarray],
    reference_vector: jnp.ndarray = jnp.array([1.0, 0.0]),
    k_stretch=1.0,
):
...
def stretching_torsional_spring_energy(
    nodal_DOFs: Tuple[jnp.ndarray, jnp.ndarray], k_stretch=1.0, k_rot=1.0
):
...
def ligament_strains_linearized(
    DOFs1: jnp.ndarray,
    DOFs2: jnp.ndarray,
    reference_vector: jnp.ndarray = jnp.array([1.0, 0.0]),
):
    ...
    axial_strain = (
        vdot(dU, reference_vector) / jnp.linalg.norm(reference_vector, axis=-1) ** 2
    )
    shear_strain = (
        jnp.cross(reference_vector, dU, axis=-1)
...
def ligament_energy_linearized(
    nodal_DOFs: Tuple[jnp.ndarray, jnp.ndarray],
    reference_vector: jnp.ndarray = jnp.array([1.0, 0.0]),
    k_stretch=1.0,
    k_shear=1.0,
    ...
    return (
        k_stretch * (axial_strain * l0) ** 2 / 2
        + k_shear * (shear_strain * l0) ** 2 / 2
        + k_rot * dRot**2 / 2
    )


def ligament_strains(
    DOFs1: jnp.ndarray,
    DOFs2: jnp.ndarray,
    reference_vector: jnp.ndarray = jnp.array([1.0, 0.0]),
):
    ...
    reference_bond_pushed = vmap(
        lambda angle, reference_v: jnp.dot(rotation_matrix(angle), reference_v),
        in_axes=(0, 0),
    )(mean_rot, jnp.ones((len(DOFs1), 2)) * reference_vector)
    reference_bond_pushed_angle = jnp.arctan2(
        *(jnp.roll(reference_bond_pushed, 1, axis=-1)).T
    )

    axial_strain = (
        vdot(current_bond_vector, current_bond_vector)
        / vdot(reference_vector, reference_vector)
    ) ** 0.5 - 1
    shear_strain = (
        jnp.mod(current_bond_angle - reference_bond_pushed_angle + jnp.pi, 2 * jnp.pi)
        - jnp.pi
    )
...
def strain_energy_bond(
    bond_connectivity: jnp.ndarray,
    bond_energy_fn: Callable = ligament_energy_linearized,
):
...
def void_angles(current_block_nodes: jnp.ndarray, bond_connectivity: jnp.ndarray):
    ...
    angles = vmap(lambda bond: compute_edge_angles(current_block_nodes, bond))(
        bond_connectivity
    )
...
def edges_distance(edge_1: jnp.ndarray, edge_2: jnp.ndarray):
    ...
    e2_onto_e1_distance = vmap(point_to_edge_distance, in_axes=(0, None))(
        edge_2, edge_1
    )
    # Compute the distance projecting first edge on the second edge
    e1_onto_e2_distance = vmap(point_to_edge_distance, in_axes=(0, None))(
        edge_1, edge_2
    )
...
def build_void_edge_distance(bond_connectivity: jnp.ndarray):
    ...
    def void_edge_distance(current_block_nodes: jnp.ndarray):
        ...
        pts1 = current_block_nodes[
            nodes_1_id // n_nodes_per_block, nodes_1_id % n_nodes_per_block
        ]
        pts1_prev = current_block_nodes[
            nodes_1_id // n_nodes_per_block, (nodes_1_id - 1) % n_nodes_per_block
        ]
        pts1_next = current_block_nodes[
            nodes_1_id // n_nodes_per_block, (nodes_1_id + 1) % n_nodes_per_block
        ]

        pts2 = current_block_nodes[
            nodes_2_id // n_nodes_per_block, nodes_2_id % n_nodes_per_block
        ]
        pts2_prev = current_block_nodes[
            nodes_2_id // n_nodes_per_block, (nodes_2_id - 1) % n_nodes_per_block
        ]
        pts2_next = current_block_nodes[
            nodes_2_id // n_nodes_per_block, (nodes_2_id + 1) % n_nodes_per_block
        ]
...
def contact_energy(
    current_void_angles: jnp.ndarray,
    min_angle: jnp.ndarray = jnp.array(0.0),
    cutoff_angle: jnp.ndarray = jnp.array(2.0 * jnp.pi / 180),
    k_contact=1.0,
):
...
def build_contact_energy(bond_connectivity: jnp.ndarray, angle_based=True):
    ...
    def contact_energy_fn(
        ...
        node_displacements = jnp.array(
            block_to_node_kinematics(block_displacement, centroid_node_vectors)
        )[:, :, :2]
        current_block_nodes = (
            block_centroids[:, None] + centroid_node_vectors + node_displacements
        )
        return jnp.sum(
            contact_energy(
                current_void_angles=distance_fn(current_block_nodes),
...
def build_strain_energy(
    ...
    def strain_energy_fn(
        ...
        node_displacements = block_to_node_kinematics(
            block_displacement, centroid_node_vectors
        )
...
def angular_momentum(block_position, block_velocity, inertia, reference_point=jnp.array([0.0, 0.0])):
    """
    Computes the angular momentum of the blocks.
    """
    momentum_centroids = jnp.cross(
        block_position[:, :2] - reference_point,
        block_velocity[:, :2] * inertia[:, :2],
        axis=-1,
    )
...
def compute_ligament_strains(block_displacement, centroid_node_vectors, bond_connectivity, reference_bond_vectors):
    ...
    return ligament_strains(
        node_displacements[bond_connectivity[:, 0]],
