...
class GeometricalParams(NamedTuple):
    ...
    These parameters are typically computed from a given geometry class defined in `mechanicalmetamaterialcloaks.geometry`.
...
class LigamentParams(NamedTuple):
    ...
    These are meant to be used with the ligament energy functions defined in `mechanicalmetamaterialcloaks.energy`.
...
class StretchingTorsionalSpringParams(NamedTuple):
    ...
    These are meant to be used with the `stretching_torsional_spring_energy` functions defined in `mechanicalmetamaterialcloaks.energy`.
...
class MagneticParams(NamedTuple):
    ...
    These are meant to be used with the magnetic energy functions defined in `mechanicalmetamaterialcloaks.energy`.
...
def load_data(path_or_filename: Union[str, Path]):
    ...
    with open(path_or_filename, "rb") as file:
        ...
        if isinstance(data, (SolutionData, EigenmodeData)):
            ...
            return class_type(
                *(
                    jnp.array(attr) if isinstance(attr, np.ndarray) else attr
                    for attr in data
                )
            )
        return data
