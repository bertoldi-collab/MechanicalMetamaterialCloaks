...
from mechanicalmetamaterialcloaks.energy import constrain_energy
from mechanicalmetamaterialcloaks.geometry import DOFsInfo, Geometry, compute_inertia
from mechanicalmetamaterialcloaks.kinematics import build_constrained_kinematics
from mechanicalmetamaterialcloaks.loading import build_loading, build_viscous_damping
    ...
    def rhs(state: jnp.ndarray, t, control_params: ControlParams, inertia: jnp.ndarray):
        ...
        return jnp.array(
            [
                velocity,
                (
                    potential_force(displacement, t, control_params)
                    + loading_fn(state, t, loading_params, damping)
                )
                / inertia,
            ]
        )
...
def build_constrained_kinematics_energy_RHS(
    ...
    constrained_energy = constrain_energy(
        energy_fn=energy_fn, constrained_kinematics=kinematics
    )
    ...
    return kinematics, constrained_energy, rhs
...
def build_reaction_force(
    ...
    kinematics, constrained_energy, rhs = build_constrained_kinematics_energy_RHS(
        geometry=geometry,
        energy_fn=energy_fn,
        loaded_block_DOF_pairs=loaded_block_DOF_pairs,
        loading_fn=loading_fn,
    ...
    def reaction_force_fn(
        ...
        if control_params.mechanical_params.inertia is None:
            ...
            _inertia = control_params.mechanical_params.inertia.reshape(
                geometry.n_blocks * 3
            )
...
def setup_dynamic_solver(
    ...
    def solve_dynamics(
        state0: jnp.ndarray, timepoints: jnp.ndarray, control_params: ControlParams
    ):
        ...
        # Solve ODE
        free_DOFs_solution = odeint(
            rhs, _state0, timepoints, control_params, _inertia, rtol=rtol, atol=atol
        )

        # Reshape solution to global state.
        displacement_history = displacement_history_fn(
            free_DOFs_solution[:, 0, :], timepoints, control_params.constraint_params
        )
...
def linear_mode_analysis(
    ...
    # Handle constraints
    kinematics = build_constrained_kinematics(
        geometry=geometry, constrained_block_DOF_pairs=constrained_block_DOF_pairs
    )
    constrained_energy = constrain_energy(
        energy_fn=energy_fn, constrained_kinematics=kinematics
    )
    ...
    all_DOFs_modes = all_DOFs_modes.at[:, free_DOF_ids].set(eigenvectors)

    # NOTE: return eigenfrequency squared and modes
    return jnp.array(eigenvalues), all_DOFs_modes.reshape(
        (len(free_DOF_ids), geometry.n_blocks, 3)
    )
